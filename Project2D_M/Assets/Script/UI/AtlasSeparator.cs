 ...
 * 최종 수정 날짜 : 2019.11.13
 * 팀             : 1 Team
 * 스크립트 용도  : 따온 리소스(아틀라스) 중 원하는 이미지를 따로 떼어놓기 위해 만들어둔 스크립트. 추후에 원하는 경로에 저장할 예정.
*/
...
public class AtlasSeparator : MonoBehaviour
{
   ...
   [SerializeField] private string m_strFolderName = null;
    ...
    public void LoadSprite(string _spritebasename)
    {
        ...
        {
            Debug.LogError("기본 아틀라스 `" + _spritebasename + "` 가 존재하지 않는다!");
            return;
        }
    ...
    void MakeFile(Sprite _sprite)
    {
        try
        {
            Debug.Log(string.Format("{0} : {1}", _sprite.name, _sprite.rect.ToString()));
